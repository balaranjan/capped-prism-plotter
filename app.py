...
if uploaded_file is not None:
    ...
    with NamedTemporaryFile(delete=False) as temp_file:
        ...
        lw = st.number_input(
            label="Enter line width (cell edges and envs):",
            min_value=1,
            max_value=10,
        ...
        for p in supercell_points:
            ...
            if not 0-cell_height*0.1 < h < cell_height*1.1:
                continue
            c = p[0][non_layer_axes]
            if any([np.all(np.allclose(c, t, rtol=5e-2)) for t in prism_and_cap_xys]):
        ...
        for height, layer_contents in capped_prism_data_by_layers.items():
            ...
            for center_coordinate, prism, edges, caps in layer_contents:
                ...
                if shade:
                    ...
                    # tx, ty = rearrange_coordinates(prism)
                    # plt.fill(tx, ty, color='r', alpha=0.5, 
                    #          edgecolor='r')
        ...
        plt.title(format_formula(title.split(',')[0].replace("~", "")) + "-type " + get_sg_symbol(temp_file_path), y=1.01, size=16)
        plt.axis('off')
        plt.tight_layout()
